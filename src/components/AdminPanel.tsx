    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('sync-dashboard-metrics', {
        body: {
          ...
          total_contracts_sum: metrics.totalContractAmount,
          debitorka_plan: metrics.totalPaymentsSum,
          debitorka_fact: metrics.completedPaymentsSum,
          company: 'Спасение',
      ...
      try {
        ...
        await supabase.functions.invoke('sync-dashboard-metrics', {
          body: {
            ...
            terminated_monthly_payment_sum: terminatedMonthlyPaymentSum,
            suspended_clients_count: suspendedClientsCount,
            suspended_contract_amount: suspendedContractAmount,
            suspended_monthly_payment_sum: suspendedMonthlyPaymentSum,
            total_contracts_sum: totalContractAmount,
            debitorka_plan: totalPaymentsSum,
            debitorka_fact: completedPaymentsSum,
            company: 'Спасение',
            user_id: selectedEmployee || user?.id || '',
            date: new Date().toISOString(),
            month: monthStr,
