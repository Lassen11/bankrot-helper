...
export const PaymentPlanBreakdownDialog = ({
  ...
  const fetchClientBreakdown = async () => {
    ...
    try {
      ...
      let planSum = 0;
      const breakdown: ClientBreakdown[] = activeClients
        ...
        .sort((a, b) => {
          // Сначала включенные в план, потом новые клиенты
  ...
  const excludedClients = clients.filter(c => !c.included_in_plan);
              ...
              {excludedClients.length > 0 && (
                <div>
                  <h3 className="font-semibold mb-3 flex items-center gap-2">
                    ...
                    <span className="text-muted-foreground text-sm">({excludedClients.length} клиентов)</span>
                  </h3>
